...
namespace vulkan
{
   ...
   application::application() :
      ...
      m_vkphysicalddevicememoryproperties{},
      m_vkphysicaldevicefeaturesEnabled{}
   {
      ...
      m_uiApiVersion                   = VK_API_VERSION_1_0; 
   ...
   application::~application()
   {
      ...
      //vkDestroySemaphore(m_vkdevice, semaphores.m_vksemaphorePresentComplete, nullptr);
      //vkDestroySemaphore(m_vkdevice, semaphores.m_vksemaphoreRenderComplete, nullptr);
   ...
   ::string_array application::get_instance_extensions()
   {
   ...
   VkResult application::createInstance()
   {
      ...
      string_array straInstanceExtensions = { VK_KHR_SURFACE_EXTENSION_NAME };

      straInstanceExtensions.appended_container(get_instance_extensions());
      ...
      if (m_straEnabledInstanceExtensions.has_elements())
      {

         for (auto & strEnabledExtension : m_straEnabledInstanceExtensions)
         {

            // Output message if requested extension is not available
            if (!m_straSupportedInstanceExtensions.contains(strEnabledExtension))
            {

               warning() << "Enabled instance extension \"" << strEnabledExtension << "\" is not present at instance level";

            }

            straInstanceExtensions.add(strEnabledExtension);
      ...
      appInfo.apiVersion = m_uiApiVersion;
   ...
   {
      vkFreeCommandBuffers(m_vkdevice, cmdPool, static_cast<uint32_t>(m_vkcommandbuffersDraw.size()), m_vkcommandbuffersDraw.data());
   }
   ...
   void application::prepare()
   {
      ...
      if (settings.overlay) {
         ...
         ui.preparePipeline(m_vkpipelinecache, m_vkrenderpass, m_colorformatFB, depthFormat);
      }
   }
   ...
   void application::updateOverlay()
   {
      ...
      io.MouseDown[0] = m_pmousestate->m_buttons.left && ui.visible;
      io.MouseDown[1] = m_pmousestate->m_buttons.right && ui.visible;
      io.MouseDown[2] = m_pmousestate->m_buttons.middle && ui.visible;
   ...
   void application::prepareFrame()
   {
      ...
      //VkResult result = swapChain.acquireNextImage(semaphores.m_vksemaphorePresentComplete, &m_uiCurrentBuffer);
   ...
   {
      //VkResult result = swapChain.queuePresent(queue, m_uiCurrentBuffer, semaphores.m_vksemaphoreRenderComplete);
   ...
   bool application::initVulkan()
   {
      ...
      m_pvulkandevice = ___new vulkan::m_vkdevice(physicalDevice);
      ...
      vkGetDeviceQueue(m_vkdevice, m_pvulkandevice->queueFamilyIndices.graphics, 0, &queue);
      ...
      //VK_CHECK_RESULT(vkCreateSemaphore(m_vkdevice, &semaphoreCreateInfo, nullptr, &semaphores.m_vksemaphorePresentComplete));
      //// Create a semaphore used to synchronize command submission
      //// Ensures that the image is not presented until all commands have been submitted and executed
      //VK_CHECK_RESULT(vkCreateSemaphore(m_vkdevice, &semaphoreCreateInfo, nullptr, &semaphores.m_vksemaphoreRenderComplete));
   ...
   void application::setupFrameBuffer()
   {
      ...
      image.extent.m_iWidth = m_iWidth;
      image.extent.m_iHeight = m_iHeight;
      ...
      VK_CHECK_RESULT(vkCreateImage(m_vkdevice, &image, nullptr, &m_colorAttachment.image));
      vkGetImageMemoryRequirements(m_vkdevice, m_colorAttachment.image, &memReqs);
      memAlloc.allocationSize = memReqs.size;
      memAlloc.memoryTypeIndex = m_pvulkandevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
      VK_CHECK_RESULT(vkAllocateMemory(m_vkdevice, &memAlloc, nullptr, &m_colorAttachment.mem));
      VK_CHECK_RESULT(vkBindImageMemory(m_vkdevice, m_colorAttachment.image, m_colorAttachment.mem, 0));
      ...
      VK_CHECK_RESULT(vkCreateImageView(m_vkdevice, &colorImageView, nullptr, &m_colorAttachment.view));
   ...
   {
      //   swapChain.create(&m_iWidth, &m_iHeight, settings.vsync, settings.fullscreen);
   }
