...
namespace vulkan1
{
   ...
   *
   */
   void Texture2D::loadFromFile(std::string filename, VkFormat format, vulkan::device* device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout, bool forceLinear)
   {
      ...
      vkGetPhysicalDeviceFormatProperties(device->m_vkphysicaldevice, format, &formatProperties);
      ...
      samplerCreateInfo.maxAnisotropy = device->m_vkphysicaldevicefeaturesEnabled.samplerAnisotropy ? device->properties.limits.maxSamplerAnisotropy : 1.0f;
   ...
   * @param (Optional) imageLayout Usage layout for the texture (defaults VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
   */
   void Texture2D::fromBuffer(void* buffer, VkDeviceSize bufferSize, VkFormat format, uint32_t texWidth, uint32_t texHeight, vulkan::device* device, VkQueue copyQueue, VkFilter filter, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout)
   {
      assert(buffer);
      ...
      bufferCopyRegion.imageSubresource.baseArrayLayer = 0;
      bufferCopyRegion.imageSubresource.layerCount = 1;
      bufferCopyRegion.imageExtent.width = m_iWidth;
      bufferCopyRegion.imageExtent.height = m_iHeight;
      bufferCopyRegion.imageExtent.depth = 1;
   ...
   *
   */
   void Texture2DArray::loadFromFile(std::string filename, VkFormat format, vulkan::device* device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout)
   {
      ktxTexture* ktxTexture;
      ktxResult result = loadKTXFile(filename, &ktxTexture);
      assert(result == KTX_SUCCESS);
   ...
   *
   */
   void TextureCubeMap::loadFromFile(std::string filename, VkFormat format, vulkan::device* device, VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout)
   {
      ktxTexture* ktxTexture;
      ktxResult result = loadKTXFile(filename, &ktxTexture);
      assert(result == KTX_SUCCESS);
