...
namespace vulkan1
{
   ...
   base_application_no_swap_chain::~base_application_no_swap_chain()
   {
      ...
      ////vkDestroySemaphore(device, m_semaphores.m_vksemaphorePresentComplete, nullptr);
      ////vkDestroySemaphore(device, m_semaphores.m_vksemaphoreRenderComplete, nullptr);
...
//   }
//
//   void base_application_no_swap_chain::create_command_buffers()
//   {
...
//   {
//      vkFreeCommandBuffers(device, m_vkcommandpool, static_cast<uint32_t>(m_vkcommandbuffersDraw.size()), m_vkcommandbuffersDraw.data());
//   }
//
...
//      vkGetDeviceQueue(device, m_pvulkandevice->queueFamilyIndices.graphics, 0, &m_vkqueue);
//
...
//      assert(validFormat);
//
//      //swapChain.setContext(instance, m_vkphysicaldevice, device);
//
...
//      //VK_CHECK_RESULT(vkCreateSemaphore(device, &semaphoreCreateInfo, nullptr, &m_semaphores.m_vksemaphorePresentComplete));
//      //// Create a semaphore used to synchronize command submission
//      //// Ensures that the image is not presented until all commands have been submitted and executed
//      //VK_CHECK_RESULT(vkCreateSemaphore(device, &semaphoreCreateInfo, nullptr, &m_semaphores.m_vksemaphoreRenderComplete));
//
...
//   // }
//   //
//   // // SRS - Window m_bResizing already handled by windowResize() in base_application_no_swap_chain::submitFrame()
...
//   // }
//   //
//   // void base_application_no_swap_chain::setSize(int m_iWidth, int m_iHeight)
//   // {
...
//      VK_CHECK_RESULT(vkAllocateMemory(device, &memAllloc, nullptr, &m_depthstencil.memory));
//      VK_CHECK_RESULT(vkBindImageMemory(device, m_depthstencil.image, m_depthstencil.memory, 0));
//
...
//      VkMemoryRequirements memReqs;
//
//      VK_CHECK_RESULT(vkCreateImage(device, &image, nullptr, &m_attachmentColor.image));
//      vkGetImageMemoryRequirements(device, m_attachmentColor.image, &memReqs);
//      memAlloc.allocationSize = memReqs.size;
//      memAlloc.memoryTypeIndex = m_pvulkandevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
//      VK_CHECK_RESULT(vkAllocateMemory(device, &memAlloc, nullptr, &m_attachmentColor.mem));
//      VK_CHECK_RESULT(vkBindImageMemory(device, m_attachmentColor.image, m_attachmentColor.mem, 0));
//
...
//      VK_CHECK_RESULT(vkCreateImageView(device, &colorImageView, nullptr, &m_attachmentColor.view));
//
//
...
//      VK_CHECK_RESULT(vkCreateFramebuffer(device, &frameBufferCreateInfo, nullptr, &m_vkframebuffers[0]));
//      //}
//   }
//
