...
namespace vulkan1
{
	...
	void ui_overlay::prepareResources()
	{
		...
		AAsset* asset = AAssetManager_open(androidApp->activity->assetManager, "Roboto-Medium.ttf"_ansi, AASSET_MODE_STREAMING);
		if (asset) {
			size_t size = AAsset_getLength(asset);
			assert(size > 0);
			char *fontAsset = ___new char[size];
		...
		auto memory = ::system()->m_papplication->file()->as_memory(filename.c_str());
		io.Fonts->AddFontFromMemoryTTF(memory.data(), memory.size(), 16.0f * scale);
#endif
		...
		device->flushCommandBuffer(copyCmd, m_vkqueue, true);
	...
	void ui_overlay::preparePipeline(const VkPipelineCache m_vkpipelinecache, const VkRenderPass m_vkrenderpass, const VkFormat colorFormat, const VkFormat m_vkformatDepth)
	{
		// Pipeline layout
