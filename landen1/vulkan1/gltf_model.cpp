...
void vkglTF::Texture::destroy()
{
	if (m_pvulkandevice)
	{
		vkDestroyImageView(m_pvulkandevice->logicalDevice, view, nullptr);
		vkDestroyImage(m_pvulkandevice->logicalDevice, image, nullptr);
		vkFreeMemory(m_pvulkandevice->logicalDevice, deviceMemory, nullptr);
		vkDestroySampler(m_pvulkandevice->logicalDevice, sampler, nullptr);
	}
}
...
void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, const ::file::path & pathFolder, vulkan::device *pdevice, VkQueue copyQueue)
{
	...
	if (!isKtx) {
		...
		vkGetPhysicalDeviceFormatProperties(m_pvulkandevice->m_vkphysicaldevice, format, &formatProperties);
		...
		VK_CHECK_RESULT(vkCreateBuffer(m_pvulkandevice->logicalDevice, &bufferCreateInfo, nullptr, &stagingBuffer));
		vkGetBufferMemoryRequirements(m_pvulkandevice->logicalDevice, stagingBuffer, &memReqs);
		...
		VK_CHECK_RESULT(vkAllocateMemory(m_pvulkandevice->logicalDevice, &memAllocInfo, nullptr, &stagingMemory));
		VK_CHECK_RESULT(vkBindBufferMemory(m_pvulkandevice->logicalDevice, stagingBuffer, stagingMemory, 0));

		uint8_t* data;
		VK_CHECK_RESULT(vkMapMemory(m_pvulkandevice->logicalDevice, stagingMemory, 0, memReqs.size, 0, (void**)&data));
		memcpy(data, buffer, bufferSize);
		vkUnmapMemory(m_pvulkandevice->logicalDevice, stagingMemory);
		...
		VK_CHECK_RESULT(vkAllocateMemory(m_pvulkandevice->logicalDevice, &memAllocInfo, nullptr, &deviceMemory));
		VK_CHECK_RESULT(vkBindImageMemory(m_pvulkandevice->logicalDevice, image, deviceMemory, 0));
		...
		m_pvulkandevice->flushCommandBuffer(copyCmd, copyQueue, true);

		vkDestroyBuffer(m_pvulkandevice->logicalDevice, stagingBuffer, nullptr);
		vkFreeMemory(m_pvulkandevice->logicalDevice, stagingMemory, nullptr);

		// Generate the mip chain (glTF uses jpg and png, so we need to create this manually)
		VkCommandBuffer blitCmd = m_pvulkandevice->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
		...
		m_pvulkandevice->flushCommandBuffer(blitCmd, copyQueue, true);
	}
	else {
		...
		if (!asset) {
			vks::tools::exitFatal("Could not load texture from "_ansi + filename + "\n\nMake sure the assets submodule has been checked out and is up-to-date."_ansi, -1);
		}
		size_t size = AAsset_getLength(asset);
		assert(size > 0);
		ktx_uint8_t* textureData = ___new ktx_uint8_t[size];
		AAsset_read(asset, textureData, size);
		AAsset_close(asset);
		result = ktxTexture_CreateFromMemory(textureData, size, KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &ktxTexture);
		delete[] textureData;
#else
		if (!vks::tools::fileExists(filename.c_str())) {
			vks::tools::exitFatal("Could not load texture from " + ::std::string(filename.c_str()) + "\n\nMake sure the assets submodule has been checked out and is up-to-date.", -1);
		}
		...
		// Get m_pvulkandevice properties for the requested texture format
		VkFormatProperties formatProperties;
		vkGetPhysicalDeviceFormatProperties(m_pvulkandevice->m_vkphysicaldevice, format, &formatProperties);
		...
		m_pvulkandevice->flushCommandBuffer(copyCmd, copyQueue);
	...
	VK_CHECK_RESULT(vkCreateSampler(m_pvulkandevice->logicalDevice, &samplerInfo, nullptr, &sampler));
	...
	VK_CHECK_RESULT(vkCreateImageView(m_pvulkandevice->logicalDevice, &viewInfo, nullptr, &view));
...
void vkglTF::Material::createDescriptorSet(VkDescriptorPool m_vkdescriptorpool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags)
{
	...
	vkUpdateDescriptorSets(m_pvulkandevice->logicalDevice, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
}
...
vkglTF::Mesh::Mesh(vulkan::device *pdevice, glm::mat4 matrix) {
	...
	VK_CHECK_RESULT(m_pvulkandevice->createBuffer(
		...
		&uniformBlock));
	VK_CHECK_RESULT(vkMapMemory(m_pvulkandevice->logicalDevice, uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
	uniformBuffer.descriptor = { uniformBuffer.buffer, 0, sizeof(uniformBlock) };
};

vkglTF::Mesh::~Mesh() {
	vkDestroyBuffer(m_pvulkandevice->logicalDevice, uniformBuffer.buffer, nullptr);
	vkFreeMemory(m_pvulkandevice->logicalDevice, uniformBuffer.memory, nullptr);
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
	...
	vkGetImageMemoryRequirements(m_pvulkandevice->logicalDevice, emptyTexture.image, &memReqs);
	memAllocInfo.allocationSize = memReqs.size;
	memAllocInfo.memoryTypeIndex = m_pvulkandevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	VK_CHECK_RESULT(vkAllocateMemory(m_pvulkandevice->logicalDevice, &memAllocInfo, nullptr, &emptyTexture.deviceMemory));
	VK_CHECK_RESULT(vkBindImageMemory(m_pvulkandevice->logicalDevice, emptyTexture.image, emptyTexture.deviceMemory, 0));
	...
	m_pvulkandevice->flushCommandBuffer(copyCmd, transferQueue);
	...
	VK_CHECK_RESULT(vkCreateSampler(m_pvulkandevice->logicalDevice, &samplerCreateInfo, nullptr, &emptyTexture.sampler));
	...
	VK_CHECK_RESULT(vkCreateImageView(m_pvulkandevice->logicalDevice, &viewCreateInfo, nullptr, &emptyTexture.view));
...
vkglTF::Model::~Model()
{
	...
	if (descriptorSetLayoutUbo != VK_NULL_HANDLE) {
		vkDestroyDescriptorSetLayout(m_pvulkandevice->logicalDevice, descriptorSetLayoutUbo, nullptr);
...
{
	vkglTF::Node *newNode = ___new Node{};
	...
	if (node.mesh > -1) {
		...
		Mesh *newMesh = ___new Mesh(m_pvulkandevice, newNode->matrix);
		...
		for (size_t j = 0; j < mesh.primitives.size(); j++) {
			...
			// Indices
			{
				...
				case TINYGLTF_PARAMETER_TYPE_UNSIGNED_INT: {
					uint32_t *buf = ___new uint32_t[accessor.count];
				...
				case TINYGLTF_PARAMETER_TYPE_UNSIGNED_SHORT: {
					uint16_t *buf = ___new uint16_t[accessor.count];
				...
				case TINYGLTF_PARAMETER_TYPE_UNSIGNED_BYTE: {
					uint8_t *buf = ___new uint8_t[accessor.count];
				...
				default:
					std::cerr << "Index component type "_ansi << accessor.componentType << " not supported!"_ansi << std::endl;
					return;
				}
			}
			Primitive *newPrimitive = ___new Primitive(indexStart, indexCount, primitive.material > -1 ? materials[primitive.material] : materials.back());
...
void vkglTF::Model::loadImages(tinygltf::Model &gltfModel, vulkan::device *pdevice, VkQueue transferQueue)
{
	for (tinygltf::Image &image : gltfModel.images) {
		...
		texture.fromglTfImage(image, m_pathFolder, pdevice, transferQueue);
...
void vkglTF::Model::loadMaterials(tinygltf::Model &gltfModel)
{
	for (tinygltf::Material &mat : gltfModel.materials) {
		vkglTF::Material material(m_pvulkandevice);
	...
	materials.push_back(Material(m_pvulkandevice));
}
	...
	for (tinygltf::Animation &anim : gltfModel.animations) {
		...
		for (auto &samp : anim.samplers) {
			...
			// Read sampler input time values
			{
				...
				float *buf = ___new float[accessor.count];
			...
			// Read sampler output T/R/S values 
			{
				...
				case TINYGLTF_TYPE_VEC3: {
					glm::vec3 *buf = ___new glm::vec3[accessor.count];
				...
				case TINYGLTF_TYPE_VEC4: {
					glm::vec4 *buf = ___new glm::vec4[accessor.count];
...
void vkglTF::Model::loadFromFile(const ::file::path & filename, vulkan::device *pdevice, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	auto str = ::system()->m_papplication->file()->as_string(filename.c_str());
	::std::string stdstr(str.c_str());

	bool fileLoaded = gltfContext.LoadASCIIFromString(&gltfModel, &error, &warning, str.begin(), str.size(), {});
	...
	if (fileLoaded) {
		if (!(fileLoadingFlags & FileLoadingFlags::DontLoadImages)) {
			loadImages(gltfModel, m_pvulkandevice, transferQueue);
		}
		...
		vks::tools::exitFatal(("Could not load glTF file \""_ansi + ::string(filename) + "\": "_ansi + ::string(error.c_str())).c_str(), -1);
		return;
	}
	...
	// Create m_pvulkandevice local buffers
	// Vertex buffer
	VK_CHECK_RESULT(m_pvulkandevice->createBuffer(
	...
	// Descriptors for per-node uniform buffers
	{
		...
		if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
			...
			VK_CHECK_RESULT(vkCreateDescriptorSetLayout(m_pvulkandevice->logicalDevice, &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo));
		}
		for (auto node : nodes) {
	...
	// Descriptors for per-material images
	{
		...
		if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
			...
			descriptorLayoutCI.bindingCount = static_cast<uint32_t>(setLayoutBindings.size());
			descriptorLayoutCI.pBindings = setLayoutBindings.data();
			VK_CHECK_RESULT(vkCreateDescriptorSetLayout(m_pvulkandevice->logicalDevice, &descriptorLayoutCI, nullptr, &descriptorSetLayoutImage));
		}
		for (auto& material : materials) {
			if (material.baseColorTexture != nullptr) {
...
void vkglTF::Model::updateAnimation(uint32_t index, float time)
{
	if (index > static_cast<uint32_t>(animations.size()) - 1) {
		std::cout << "No animation with index "_ansi << index << std::endl;
		return;
	}
...
void vkglTF::Model::prepareNodeDescriptor(vkglTF::Node* node, VkDescriptorSetLayout descriptorSetLayout) {
	if (node->mesh) {
		...
		vkUpdateDescriptorSets(m_pvulkandevice->logicalDevice, 1, &writeDescriptorSet, 0, nullptr);
	}
	for (auto& child : node->children) {
